...
import { TagRelationPage } from '../tag/TagRelationPage/TagRelationPage'
...
function RelationDetailPage(): ReactElement {
  ...
  if (snapshot.kind === 'tag') {
    return <TagRelationPage key={relationId} snapshot={snapshot} />
  }
...
function RelationEditor({ snapshot }: RelationEditorProps): ReactElement {
  ...
  const editor = useRelationEditor({
    ...
    onRemoved: () => navigate({ to: '/relations', search: true }),
  })
